<template>
  <div>
    <h1 id="this-title" class="mb-3 font-bold text-3xl">Contacts</h1>
    ...
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="contact in contacts.data" :key="contact.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <inertia-link class="px-4 py-3 flex items-center focus:text-indigo-500" :href="route('contacts.edit', contact.id)">
              {{ contact.name }}
              <icon v-if="contact.deleted_at" name="trash" class="flex-shrink-0 w-3 h-3 fill-gray-400 ml-2" />
            </inertia-link>
          </td>
          <td class="border-t">
            <inertia-link class="px-4 py-3 flex items-center" :href="route('contacts.edit', contact.id)" tabindex="-1">
        ...
        <tr v-if="contacts.data.length === 0">
          <td class="border-t px-4 py-3" colspan="4">No contacts found.</td>
        </tr>
      </table>
    </div>
